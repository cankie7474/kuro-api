    ...
    <div class="sl-stack sl-stack--vertical sl-stack--1 sl-flex sl-flex-1 sl-flex-col sl-items-stretch sl-max-w-full sl-ml-2 @if($expandable) sl-cursor-pointer @endif">
        <div class="sl-flex sl-items-center sl-max-w-full" style="gap: 0.65rem; flex-wrap: wrap;">
            ...
            @unless($isArrayBody)
                ...
                @endif
                @if($deprecated)
                    <span class="sl-px-2 sl-py-0.5 sl-rounded sl-border sl-text-warning" style="font-size: 11px;">deprecated</span>
                @endif
            @endunless
        </div>
        @if($description)
            ...
            <div class="sl-stack sl-stack--horizontal sl-stack--2 sl-flex sl-flex-row sl-items-baseline sl-text-muted">
                ...
                <div class="sl-flex sl-flex-1 sl-flex-wrap" style="gap: 4px;">
                    <div class="sl-max-w-full sl-break-all sl-px-1 sl-bg-canvas-tint sl-text-muted sl-rounded sl-border">
                        {{ is_array($example) || is_bool($example) ? json_encode($example, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : $example }}
                    </div>
                </div>
            </div>
        @endif

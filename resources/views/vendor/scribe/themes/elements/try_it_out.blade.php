                    ...
                    <div class="sl-panel__content-wrapper sl-bg-canvas-100 children" role="region">
                        @if($endpoint->hasJsonBody())
                            <div class="TextRequestBody sl-p-4">
                                ...
                                >{!! json_encode($endpoint->getSampleBody(), JSON_PRETTY_PRINT | JSON_HEX_TAG | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) !!}</div>
                            </div>
                        @else
                            <div class="ParameterGrid sl-p-4">
                                        ...
                                        <div class="sl-input sl-flex-1 sl-relative">
                                            ...
                                            @elseif(str_ends_with($parameter->type, '[]'))
                                                <input aria-label="{{ $name }}" name="{{ $name }}"
                                                       id="bodyparam-{{ $endpoint->endpointId() }}-{{ $name }}"
                                                       placeholder="{{ $parameter->description }}"
                                                       value="{{ json_encode($parameter->example, JSON_HEX_TAG | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) }}" data-component="body"
                                                       class="sl-relative sl-w-full sl-h-md sl-text-base sl-pr-2.5 sl-pl-2.5 sl-rounded sl-border-transparent hover:sl-border-input focus:sl-border-primary sl-border"
                                                >
                                            @else
                                                <input aria-label="{{ $name }}" name="{{ $name }}"
